...
/*
	...
	if(i_number_ao_subdevices == 0)
	{
		printf("****    No AO sub-devices found in system   ****\n\n");
        goto error;
	};
*/
...
//// ************The following is the configuration and the main output part.************** ////
	...
	{
		int index_subdevice;
		// choosing the subdevice, needless so commented out
		// notice: index_subdevice should be iterated out!!!
/*		if(i_number_ao_subdevices > 1)
		{
			while(true)
			{
				...
				printf("\nYour choice (1 - %2d) (RETURN to terminate): ", i_number_ao_subdevices);
				...
				if( (index_subdevice < 1)||(index_subdevice > i_number_ao_subdevices) )
				{
					continue;
				}
